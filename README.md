...
- 정상적인 로또 구입금액을 투입하면 로또 발급 단계로 넘어갈 수 있다.
  - [x] 로또 발급버튼, 자동/수동 매수, 로또용지 추가버튼이 표시된다.
  - [x] 전체 자동구매를 기본값으로 한다.
- 로또용지를 추가해서 수동구매를 할 수 있다.
  - [x] 용지추가 버튼을 누를 때마다 로또번호 선택용지가 한 장씩 추가된다.
  - [x] 자동구매 수량이 0매이면 용지추가 버튼이 비활성화 된다.
  - [x] 로또용지에서 6개 보다 적게 고르면 상태메세지가 화면에 표시된다.
  - [x] 로또용지에서 6개 보다 적게 고르면 로또 발행버튼이 비활성화 되어있다.
- 각 로또용지 별로 적용수량을 조정할 수 있다.
  - [x] 새로 추가한 로또용지의 적용수량의 기본값은 1매이다.
  - [x] 자동구매 수량이 남아있을 경우, 적용수량을 조정해서 수동구매로 전환할 수 있다.
  - [x] 로또용지 삭제버튼을 누르면 해당 로또용지가 사라지고 해당 적용수량은 자동구매로 전환된다.
- 로또복권 발행 버튼을 누르면 로또를 발행을 진행한다.
  - [x] 자동구매 매수, 수동구매 매수가 적힌 안내메세지를 Confirm으로 표시한다.
  - [x] '예'를 선택할 경우에만 로또복권을 발행한다.
